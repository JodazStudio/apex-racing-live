...
import RaceStats from "@/components/dashboard/RaceStats";
import LiveLeaderboard from "@/components/dashboard/LiveLeaderboard";
import FinishLineTicker from "@/components/dashboard/FinishLineTicker";
    ...
    const interval = setInterval(() => {
      ...
      setAthletes(prev => prev.map(athlete => {
        ...
        if (newDistance >= MAX_DISTANCE) {
          ...
          toast({
             title: "¡NUEVA LLEGADA!",
             description: `${athlete.name} ha cruzado la meta con un tiempo de ${finalTime}`,
             duration: 5000,
          });
        }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      finishersList: finished.sort((a, b) => {
          // Sort by finishing time or distance if not finished
      ...
      <main className="container mx-auto px-4 py-8 mt-16 max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            ...
            <h1 className="font-satoshi text-4xl font-black uppercase tracking-tight md:text-5xl">
              Panel de <span className="text-primary underline decoration-primary/20 underline-offset-8 italic">Control</span>
            </h1>
          </div>
